...
BILATERAL_SIGMA_INTENSITY = 0.001  # Small for preserving sharp features
...
def setup_publication_subplot(ax, elev, azim, show_xlabel=True, view_name=""):
    ...
    if z_range > 0:
        ...
        # More compact format for narrow view
        ax.set_zticklabels([f'{tick:.2f}' for tick in z_ticks])  # Changed from .3f to .2f
    ...
    ax.tick_params(axis='x', labelsize=10, pad=5, direction='out', length=4)
    ax.tick_params(axis='y', labelsize=10, pad=5, direction='out', length=4)
    ax.tick_params(axis='z', labelsize=10, pad=12, direction='out', length=4)  # Increased z-axis pad
...
if GUI_MODE:
    ...
    print("  • 'a': Auto-fit axes to data bounds")
    ...
    def update_tick_format():
        ...
        if state['current_view_mode'] == 'narrow':
            ...
            ax.tick_params(axis='z', labelsize=9, pad=15)  # Even more pad for narrow
            # Re-format z-tick labels if they exist
            zticks = ax.get_zticks()
            if len(zticks) > 0:
                ax.set_zticklabels([f'{tick:.2f}' for tick in zticks])
        else:
